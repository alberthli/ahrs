...
void LSM9DS0::madgwickFilterUpdate() {
	...
	while(true) {
		...
		for (int i; i<iter; i++) {
			...
			grad_SEq[0] -= BETA * SEqhatdot0 *step;
			grad_SEq[1] -= BETA * SEqhatdot1 *step;
		...
		SEq[0] += (SEqdot0 ) * dt + grad_SEq[0];
		SEq[1] += (SEqdot1 ) * dt + grad_SEq[1];
		SEq[2] += (SEqdot2 ) * dt + grad_SEq[2];
		SEq[3] += (SEqdot3 ) * dt + grad_SEq[3];
